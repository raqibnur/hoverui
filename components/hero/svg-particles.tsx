/*
 * VENDORED THIRD-PARTY CODE — not written to this repo's standards, and deliberately not
 * linted to them. The disables below are scoped to this file and each one is load-bearing
 * upstream, not sloppiness we could tidy:
 *
 *   ban-ts-comment   the `@ts-nocheck` is upstream's; retyping 500 lines of physics we did
 *                    not write would guarantee the next `originkit add` conflicts with it.
 *   react-hooks/refs `physicsRef.current` and `samplingRef.current` are rewritten during
 *                    render on purpose: the rAF loop closes over them once and reads the
 *                    latest props every frame without re-subscribing. Moving the writes into
 *                    an effect is the "correct" shape but changes when the loop observes a
 *                    prop change, which is exactly the timing this component is tuned on.
 *   no-unused-vars   an upstream `catch (_)`.
 *
 * The rule this file does NOT get an exemption from is CLAUDE.md rule 2: it imports nothing
 * but React. Anything that changes that has to be cut, not disabled.
 */
/* eslint-disable @typescript-eslint/ban-ts-comment, react-hooks/refs, @typescript-eslint/no-unused-vars */
// @ts-nocheck
"use client";

/**
 * SVG Particle — vendored from the Originkit registry (`originkit add svgparticles`).
 *
 * WHY IT LIVES HERE AND NOT IN `registry/hover/`
 *
 * This is not one of the frozen 12 and it is not a registry item. It never enters
 * `registry.json`, never builds to `public/r`, and is never installed by a consumer — it is
 * site chrome for this repo's own hero, in the same category as
 * `components/gallery/charge-field.tsx`. SCOPE.md's v1 list is therefore untouched: the
 * registry still ships exactly twelve effects.
 *
 * It also imports nothing but React, so CLAUDE.md rule 2 holds — there is no runtime
 * dependency here, only a lot of vendored source.
 *
 * WHY IT IS NOT CLEANED UP
 *
 * The algorithm is kept byte-close to upstream so a future `originkit add` can be diffed
 * against it. Only editor-only scaffolding was removed: the `@framer*` layout annotations,
 * the hard-coded remote default image (an image is now required), and the "Upload image in
 * panel" placeholder, which pointed at a Framer inspector that does not exist here. The
 * `@ts-nocheck` is upstream's and is kept deliberately rather than retyping 500 lines of
 * physics we did not write — see `components/hero/tailwind-logotype.tsx` for the typed
 * boundary this is used through.
 *
 * The one thing this component does NOT do is honour `prefers-reduced-motion`, and it drives
 * a permanent `requestAnimationFrame` loop whether or not anything is moving. Both are
 * handled at the boundary, which mounts this only for a fine pointer with motion allowed.
 */
...
export default function SvgParticles(__props) {
  const {
    imageConfig,
    particleCount,
    particleSize,
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Smooth the repulsion zone position each frame. The floor of 0.08 keeps
      // the zone moving as a single continuous shape rather than stamping
      // discrete rings at dozens of positions during a fast sweep.
      ...
      for (const p of particles) {
        ...
        if (repOn) {
          if (rMode === "random") {
            ...
            if (dist < repCutoff) {
              if (!p.inZone) {
                // Random displacement in any direction — some particles move a
                // lot, some a little, no circle constraint so it feels organic.
                const angle = Math.random() * Math.PI * 2;
