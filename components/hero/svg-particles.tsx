/*
 * VENDORED THIRD-PARTY CODE — from the Originkit registry, not written to this repo's
 * standards and deliberately not linted to them. Each exemption is load-bearing upstream:
 *
 *   ban-ts-comment    the `@ts-nocheck` below is upstream's. Without it `next build` fails
 *                     with 151 implicit-any errors from physics this repo did not write.
 *   no-explicit-any   upstream's casts on imageConfig / hoverConfig.
 *   react-hooks/refs  `physicsRef.current` and `samplingRef.current` are rewritten during
 *                     render on purpose, so the rAF loop reads the latest props every frame
 *                     without re-subscribing.
 *
 * Retyping any of it would guarantee the next `originkit add` conflicts with the result. The
 * one rule this file does NOT get an exemption from is CLAUDE.md rule 2: it imports nothing
 * but React.
 */
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-explicit-any, react-hooks/refs */
// @ts-nocheck
"use client"
